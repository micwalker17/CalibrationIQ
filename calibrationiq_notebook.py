...
# These simulate the inputs that would be automatically extracted from a Jira ticket.
...
selected_pdf_base64 = ""
selected_pdf_filename = "sample_cal_cert.pdf" # Placeholder for local run
caliper_data = {}
no_measurements_found = True # Default to True, assuming no data until loaded
...
# ============================================================================
print("\nBLOCK 2: PDF DATA SIMULATION")
# This simulates finding and encoding a PDF file.
try:
...
# A hardcoded JSON response makes the project runnable without a live AI service.
# ============================================================================
print("\nBLOCK 3: AI-POWERED DATA EXTRACTION SIMULATION")
# This simulates the JSON response from the Gemini model.
...
# physical impact on measurements (e.g., does it make parts seem bigger or smaller?).
# ============================================================================
print("\nBLOCK 4: DEVIATION CALCULATION & VALIDATION")
...
try:
    ...
    print(f"✅ Deviation calculated: {deviation_value_inches:+.6f} {units} (Caliper reads {direction})")
except Exception as e:
...
# ============================================================================
print("\nBLOCK 5 & 6: HISTORICAL DATA SIMULATION")
# In a real scenario, you would connect to Spark. Here, we simulate it.
try:
    ...
    spark = None # Gracefully handle no spark
...
def generate_sample_dataframe(spark_session):
    ...
    if not spark_session:
        ...
        return None
    sample_data = [
        ("WO-001", "SN-101", "Char 1", "Hole Diameter", 0.5005, 0.5000, 0.5010, 0.4990, "BILATERAL", "Critical"),
        ("WO-001", "SN-101", "Char 2", "Step Height", 1.2510, 1.2500, 1.2520, 1.2480, "BILATERAL", "Major"),
        ("WO-002", "SN-201", "Char 5", "Outer Diameter", 3.0001, 3.0000, 3.0005, 2.9995, "BILATERAL", "NotSpecified"),
        ("WO-002", "SN-201", "Char 6", "Groove Depth", 0.1008, 0.1000, 0.1010, 0.0990, "BILATERAL", "NotSpecified"), # This will fail
        ("WO-003", "SN-301", "Char 9", "Slot Width", 0.7511, 0.7500, 0.7510, 0.7490, "BILATERAL", "Minor"), # This will be saved
    ]
    schema = ["job_number", "sample_serial_number", "dimension_id", "feature_name", "measured_value", "nominal_value", "original_upper_tol", "original_lower_tol", "tolerance_type", "criticality"]
...
# part dimensions and determines the final pass/fail status, including business logic.
# ============================================================================
print("\nBLOCK 7: ADJUSTED VALUE CALCULATION & IMPACT ANALYSIS")
if not no_measurements_found and all_measurements_df:
    from pyspark.sql.functions import col, lit, when
    all_measurements_df = all_measurements_df.withColumn("adjusted_value", col("measured_value") - lit(deviation_value_inches))
    all_measurements_df = all_measurements_df.withColumn("allowance_eligible", when(col("criticality").isin(["Critical", "Major"]), lit("NO - KC")).otherwise(lit("YES")))
    all_measurements_df = all_measurements_df.withColumn("expanded_upper_tol", when(col("allowance_eligible") == "YES", col("original_upper_tol") + ((col("original_upper_tol") - col("nominal_value")) * 0.20)).otherwise(col("original_upper_tol")))
    all_measurements_df = all_measurements_df.withColumn("expanded_lower_tol", when(col("allowance_eligible") == "YES", col("original_lower_tol") - ((col("nominal_value") - col("original_lower_tol")) * 0.20)).otherwise(col("original_lower_tol")))
    all_measurements_df = all_measurements_df.withColumn("final_status", when((col("adjusted_value") >= col("expanded_lower_tol")) & (col("adjusted_value") <= col("expanded_upper_tol")), lit("✅ PASS")).otherwise(lit("❌ FAIL")))
...
if failure_count > 0:
    ...
    print("   -> Next steps would be: generate HTML report, create NC, post to Jira.")
else:
    print("✅ Simulation Complete: No failures were identified.")
    print("   -> Next step would be: post 'All Clear' comment to Jira and close ticket.")
